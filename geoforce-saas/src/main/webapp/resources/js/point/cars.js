...
Cars.search = function() {	
    ...
    $.ajax({
        ...
        success: function(e){
            Dituhui.hideMask();
        	if(e && e.result && e.result.total > 0) {
        		Cars.data_cars = e.result;
        		Cars.showCars();
        		return;
        	}
            Dituhui.showPopover("查询到0条车辆数据");
            Cars.clear();
        },
        error: function(){
            Dituhui.hideMask();
...
Cars.showCars = function(){
	var cars = Cars.data_cars.rows.concat();
	var len = cars.length;
	var h = '';
	for(var i=0; i<len; i++) {
		var car = cars[i];
    ...
    var html_page = Dituhui.setPage(Cars.data_cars.records, Cars.data_cars.page-1, '\'pager_cars\''); 
        ...
        success: function(e){
            if(e.isSuccess && e.result) {
                ...
                if( e.result.length === 0 ) {
					Dituhui.showPopover("未查询到车辆类型数据");
                }
                else {
