...
Cluster.prototype.isMarkerAlreadyAdded_ = function (marker) {
  ...
  if (this.markers_.indexOf) {
    ...
    for (i = 0; markerTmp = this.markers_[i]; i++) {
      if (marker === markerTmp) {
        return true;
      }
    }
...
MarkerClusterer.prototype.onRemove = function () {
  ...
  for (i = 0; marker = this.markers_[i]; i++) {
	  marker.setMap(this.activeMap_);
  }

  // Remove all clusters:
  for (i = 0; cluster = this.clusters_[i]; i++) {
	  cluster.remove();
  }
  this.clusters_ = [];
...
MarkerClusterer.prototype.repaint = function () {
  ...
  setTimeout(function () {
    var i,cluster;
    for (i = 0; cluster = oldClusters[i]; i++) {
    	cluster.remove();
    }
  }, 0);
};
...
//update by ws
MarkerClusterer.IMAGE_PATH = ctx+"/images/mapcloud/poiCluster";
