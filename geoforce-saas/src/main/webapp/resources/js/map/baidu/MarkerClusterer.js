...
(function(){
    ...
    MarkerClusterer.prototype._createClusters = function(){
        ...
        //update by ws
        var items = this._markers.concat();
        var that = this;
        setTimeout(function(){
            var start = +new Date(); // 返回当前时间戳
            do {
//            	var marker = items.shift();
            	var markers = items.splice(0, 20);
            	for(var i=0,marker;marker = markers[i]; i++){
            		if(marker && !marker.isInCluster && extendedBounds.containsPoint(marker.getPosition()) ){
                		that._addToClosestCluster(marker);
                		if(marker.attributes && marker.attributes.mLabel){
                    		marker.setLabel(marker.attributes.mLabel);
                    	}
                	}
            	}
...
//                //处理marker标签label 聚合后消失问题 add by ws
//                if(marker.attributes && marker.attributes.mLabel){
//            		marker.setLabel(marker.attributes.mLabel);
//            	}
//                //end
...
//        var distance = 4000000;//update by ws
    	var distance = 4000;
...
//                var d = this._map.getDistance(center, position);
            	var d = this.distanceBetweenPoints(center, position);//update by ws
                if(d < distance){
                    distance = d;
...
//        }
    ...
    Cluster.prototype.updateClusterMarker = function () {
        ...
        this._clusterMarker.addEventListener("click", function(event){
        	//update by ws
            var textIconLayer = event.currentTarget;
            var count = 50;
            if(textIconLayer){
            	var document = textIconLayer._domElement;
            	if(document.innerHTML){
            		count = parseInt(document.innerHTML);
            	}
            }
            if(count <= 50){
