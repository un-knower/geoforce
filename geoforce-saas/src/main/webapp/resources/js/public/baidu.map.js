...
Baidu.init = function() {
    ...
    /*
    console.log( 12944304.54, 4872906.363 );
    var c1 = Baidu.getCoord(12944304.54, 4872906.363);
    console.log('baidu: ' + c1.x + ',' + c1.y );
    console.log('**************');
    console.log( 12946433.4,4861399.248 );
    var c2 = Baidu.getCoord( 12946433.4,4861399.248);
    console.log('baidu: ' + c2.x + ',' + c2.y );*/
}
/**
 * 切换到百度地图
 */
 ...
 * @param - x 超图坐标x
 * @param - x 超图墨卡托坐标y
 * @param - coordtype 超图坐标类型，不传时 默认为墨卡托，"meters"为墨卡托，"lonlat"为经纬度
 * @return - 返回百度墨卡托坐标
 */
Baidu.getCoord = function (x, y, coordtype) { 
    var lonlat;
    if(typeof(coordtype) == "undefined" || coordtype == "meters") {
    	lonlat = Dituhui.metersToLatLon(new SuperMap.LonLat(x, y));
    }
    else {
 ...
 * @param - x_b 百度墨卡托x
 * @param - y_b 百度墨卡托y
 * @param - coordtype 需要返回的超图坐标类型，不传时 默认为墨卡托，"meters"为墨卡托，否则为经纬度
 * @return - 返回超图墨卡托坐标
 */
Baidu.restoreCoord = function (x_b, y_b, coordtype) {  
    ...
    if(typeof(coordtype) == "undefined" || coordtype == "meters") {
    	var c = Dituhui.latLonToMeters(new SuperMap.LonLat(gcjLon, gcjLat));
    	return {'y' : c.lat, 'x' : c.lon};
    }
    return new SuperMap.LonLat(gcjLon, gcjLat);
}
...
Baidu.redrawBoundry = function() {
    ...
    for(var i=fs.length; i--; ) {
        ...
        var geometry = Dituhui.Zone.DrawRegion(attr.geo.parts, points);
