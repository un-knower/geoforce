...
public class CommonUtil {
	...
	public static final String TELEPHONE_PATTERN = "\\+*\\d+";
	/**
	 * 
	 * <p>Title ：isTelephone</p>
	 * Description：		判断该字符串是否为电话号码
	 * @param telphone
	 * @return
	 * Author：Huasong Huang
	 * CreateTime：2015-7-10 上午10:51:38
	 */
	public static boolean isTelephone(final String telphone){
		boolean returnStatus = false;
		if(!isStringEmpty(telphone)){
			String tempName = telphone.toLowerCase();
			returnStatus = tempName.matches(TELEPHONE_PATTERN);
		}
		return returnStatus;
	}
	 ...
	 */
	public static final String dateConvert(Date date,DateType type){
		String result = null;
		try{
			if(DateType.DATE_TYPE.equals(type)){
